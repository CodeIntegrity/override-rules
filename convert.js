...
(() => {
  ...
  function buildCountryProxyGroups({
    ...
    for (const country of countries) {
      ...
      const effectiveType = groupType === 3 && excludeSet.has(country) ? smartFallback : groupType;
  ...
  function getSortedCountries(countryInfo, minCount) {
    return countryInfo.filter((item) => item.nodes.length >= minCount).sort((a, b) => {
